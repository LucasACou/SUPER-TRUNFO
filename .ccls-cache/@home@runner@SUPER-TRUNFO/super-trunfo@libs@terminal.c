...
void resetar_fonte() { printf("\033[0m"); }

void definir_propriedade_da_fonte(int property) {
  printf("\033[%dm", property);
}

void exibir_cursor() { printf("\033[?25h"); }
void ocultar_cursor() { printf("\033[?25l"); }
void mover_cursor_xy(int x, int y) { printf("\033[%d;%dH", y, x); }
void limpar_tela() {
